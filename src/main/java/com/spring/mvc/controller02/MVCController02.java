...
public class MVCController02 {
    ...
    // localhost:포트번호(기본값8080)/chap02/hobbies
    @RequestMapping("/hobbies")
    public String hobbies(Model model){ //.jsp로 자바 변수를 보내려면 Model을 선언부에 선언해준다
        String name = "룰루";
    ...
    // ModelAndView 방식 컨트롤러 메소드는 리턴자료형이 ModelAndView 객체
    @GetMapping("/hobbies2")
    public ModelAndView hobbies2(){
        // .jsp파일(view단) 으로 보내고 싶은 자료 설정
        String name = "참참이";
        List<String> hList = List.of("전깃줄에서 꾸벅꾸벅 졸기","쌀줍줍","짹짹소리내기");

        // 목적지 뷰 이름 적기(포워딩 방식)
        //         /WEB-INF/views/chap02/hobbies.jsp
        String viewName = "/chap02/hobbies";

        // ModelAndView 객체 생성 및 세팅
